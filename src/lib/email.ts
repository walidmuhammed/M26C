...
export const sendWelcomeEmail = async (userEmail: string) => {
  const html = `
    <div style="font-family: Montserrat, sans-serif; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #ff0000;">Welcome to M26C!</h1>
      ...
      <h2>Need Help?</h2>
      <p>If you have any questions, please contact our support team at support@m26c.com</p>
  ...
  try {
    const info = await transporter.sendMail({
      ...
      to: userEmail,
      subject: "Welcome to M26C Platform",
      messageId: `<${Date.now()}-welcome@m26c.com>`,
      headers: {
        Precedence: "bulk",
      },
      html,
...
Need Help?
If you have any questions, please contact our support team at support@m26c.com
...
export const sendAdminNotification = async (userEmail: string) => {
  const html = `
    <div style="font-family: Montserrat, sans-serif; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #ff0000;">New Subscriber Registration</h1>
      <h2>Registration Details</h2>
      <ul>
  ...
  try {
    const info = await transporter.sendMail({
      ...
      messageId: `<${Date.now()}-admin@m26c.com>`,
      priority: "high",
      html,
      text: `
